...
lexis_firm_names_clean.squeeze().to_csv('C:/Users/Jakob/Documents/financial_news_data/lexis_firm_names_clean.csv', index=False)
...
rels = pd.read_pickle(os.path.join(output_path, 'lexisnexis_rel_database_full.pkl'))
...
# remove duplicate relationships (same participants, same type, same year)
rels = rels[['firm_a', 'firm_b', 'year', 'rels_pred']]
rels = rels.groupby(['firm_a', 'firm_b', 'year']).agg(list)
rels.reset_index(inplace=True)
...
for rel_name in important_labels:
    rels[rels.rels_pred ==rel_name].drop(columns=['rels_pred']).to_csv(
...
df_static.to_csv('C:/Users/Jakob/Documents/financial_news_data/lexis_match_orbis2023_compustat_static.csv', index=False)
...
df_dynamic.to_csv('C:/Users/Jakob/Documents/financial_news_data/lexis_match_orbis2023_compustat_dynamic.csv', index=False)
...
rnd_ids = pd.concat([rnd.firm_a, rnd.firm_b]).drop_duplicates()

# count number of links per firm
pd.concat([rnd.firm_a, rnd.firm_b]).value_counts().median()
df_static[df_static.ID=='JP2011001026329'].name_internat
df_static.columns
compustat = df_static[df_static.from_compustat==1]
compustat_us = compustat[compustat.country_iso_code=='USA']
...
df.rels_pred.explode().value_counts()